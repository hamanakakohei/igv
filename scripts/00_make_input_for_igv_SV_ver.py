...
def get_end_and_svtype(variant):
    """
      cyvcf2.VCFで読んだvcfのvariantから、end位置とタイプを返す
      - DEL/DUP/INV: INFO列のENDを使う（end_chromはchromと同じにする）
      - BND: ALT列からend_chrom, endを取って使う
    """
...
def determine_regions(chrom, start, end_chrom, end, svtype,
                      small_size, medium_size, extend_ratio, margins):
    """
      SVタイプとサイズに応じて撮影するchr, start, end, nameのリスト（1つのSVで複数撮影する）を返す
    """
    regions = []
    if svtype == "BND":
        for m in margins:
            regions.append((chrom, start - m, start + m, f"{chrom}_{start}_{end_chrom}_{end}_{svtype}_sta_margin{m}"))
            regions.append((end_chrom, end - m, end + m, f"{chrom}_{start}_{end_chrom}_{end}_{svtype}_end_margin{m}"))
        return regions

    size = abs(end - start)
    if size <= small_size:
        regions.append((chrom, int(start - size * extend_ratio), int(end + size * extend_ratio), f"{chrom}_{start}_{end}_{svtype}_ratio{extend_ratio}"))
    elif size <= medium_size:
        regions.append((chrom, int(start - size * extend_ratio), int(end + size * extend_ratio), f"{chrom}_{start}_{end}_{svtype}_ratio{extend_ratio}"))
        for m in margins:
            regions.append((chrom, start - m, start + m, f"{chrom}_{start}_{end}_{svtype}_sta_margin{m}"))
            regions.append((chrom, end   - m, end   + m, f"{chrom}_{start}_{end}_{svtype}_end_margin{m}"))
    else:
        for m in margins:
            regions.append((chrom, start - m, start + m, f"{chrom}_{start}_{end}_{svtype}_sta_margin{m}"))
            regions.append((chrom, end   - m, end   + m, f"{chrom}_{start}_{end}_{svtype}_end_margin{m}"))
...
def main():
    parser = argparse.ArgumentParser(description="Prepare IGV snapshot regions for each family. SV用に作ったが、SNVでもmarginやらを調整して撮影できる")
    ...
    parser.add_argument("--call-dir", required=True, help="call-dir/proband_id/proband_id.fam.vep.annot.AD_XL.rare.txtというパスに結果があると想定している")
    ...
    parser.add_argument("--margin", type=int, nargs="+", default=[1000, 5000])
    ...
    for _, row in probands.iterrows():
        ...
        for sv_id in sv_df["ID"]:
            ...
            chrom, start, end_chrom, end, svtype = get_end_and_svtype(variant)
